...
class ActorSheetConan2d20Character extends ActorSheetConan2d20 {
    ...
    getData() {
        ...
        // Update wounded icon
        sheetData.data.health.physical.wounds = C2Utility.addDots(duplicate(sheetData.data.health.physical.wounds), sheetData.data.health.physical.wounds.max);

        // Update traumatized icon
        sheetData.data.health.mental.traumas = C2Utility.addDots(duplicate(sheetData.data.health.mental.traumas), sheetData.data.health.mental.traumas.max);
        ...
        if (sheetData.actor.inventory.weapon.items.filter(i => i.data.group === "shield").length > 0) {
           ...
           sheetData.data.armor = C2Utility.calculateArmor(sheetData.actor.inventory.armor.items, shields);
        } else {
            sheetData.data.armor = C2Utility.calculateArmor(sheetData.actor.inventory.armor.items, undefined);
        }
    ...
    _prepareItems(actorData) {
        const inventory = {
            ...
            transportation: { label: game.i18n.localize("CONAN.transpoHeader"), items: [] }
        };
    ...
    activateListeners(html) {
        ...
        html.find(".condition-value").mouseup(ev => {
            const  condKey = $(ev.currentTarget).parents(".sheet-condition").attr("data-cond-id")
            if (ev.button === 0) {
